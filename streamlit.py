...
db_config = {
    'user': 'root',
    'password': '',
    'host': 'localhost',  # corrected from 'local_host' to 'localhost'
...
try:
    ...
    st.info(f"The current temperature is {temp} °C.\n"
            f"The wind speed is {speed} m/s.\n"
            f"The wind is coming from {common_dir}.\n"
            f"The air pressure is {pressure} hPa.")
except requests.exceptions.RequestException as e:
    st.error(f"Failed to retrieve current weather data: {e}")

# Fetching data for the machine learning model from MySQL
query = f"SELECT * FROM measurements WHERE city='Jakarta' AND country='Indonesia'"
data_for_model = fetch_data(query)

# Preprocess the data as required by your model
if not data_for_model.empty:
    # Assuming 'data_for_model' has the necessary columns and is in the correct format
...
with st.spinner('Loading...'):
    try:
        ...
        st.error(f"Failed to retrieve hourly weather data: {e}")
